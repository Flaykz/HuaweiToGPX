...
NAME = 'exportGPX'
DESCRIPTION = 'a command line tool to export Huawei tracking files to GPX files'
DEBUG = False
...
def normalize_timestamp(timestamp):
    if str(timestamp).find('E12') >= 0 or len(str(int(float(timestamp)))) == 13:
...
def point(infos):
    if str(infos.get('lat', '0')) == "90.0" and str(infos.get('lon', '0')) == "-80.0":
        return "\r\n\
        </trkseg>\r\n\
        <trkseg>"
    else:
        return "\r\n\
            <trkpt lat=\""+ str(infos.get('lat', '0')) + "\" lon=\"" + str(infos.get('lon', '0')) + "\">\r\n\
                <ele>" + str(infos.get('alt', '0')) + "</ele>\r\n\
                <time>" + str(sec_to_datetime(infos.get('t', 0))) + "</time>\r\n\
                <extensions>\r\n\
                    <gpxtpx:TrackPointExtension>\r\n\
                        <gpxtpx:hr>" + str(int(infos.get('hr', 0))) + "</gpxtpx:hr>\r\n\
                        <gpxtpx:cad>" + str(infos.get('vitesse', 0)) + "</gpxtpx:cad>\r\n\
                    </gpxtpx:TrackPointExtension>\r\n\
                </extensions>\r\n\
            </trkpt>"
...
def process(fileIn):
    if fileIn.find('HiTrack_') >= 0:
        ...
        print('You have to give a original HiTrack_ file in entry, not : ' + fileIn)
        return None
...
#                 dic['alt'] = float(infos[4].split('=')[1])
                ...
                if typeData == 'p-m':
                    ...
                    dic['pace'] = str(int(v) // 60) + "'" + str(int(v) % 60) + '"'
                    pm.append(dic)
                elif typeData == 'b-p-m':
                    #Beat per minutes ?
                    dic['k'] = k
...
def main():
    ...
    parser.add_argument("input_file", nargs='*', default=os.getcwd(), help="HiTrack file to convert to GPX")
